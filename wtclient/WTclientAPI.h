...
//
_declspec(dllexport) HRESULT  WINAPI ReadOPCItemFromDevice (HANDLE hConnect, HANDLE hGroup, HANDLE hItem, VARIANT *pVar, FILETIME *pTimeStamp, DWORD *pQuality);

//
// ReadOPCItemFromCache(...)
//
// Uses the SyncIO Interface to read an opc item directly from the Server's Cache.
//
_declspec(dllexport) HRESULT  WINAPI ReadOPCItemFromCache (HANDLE hConnect, HANDLE hGroup, HANDLE hItem, VARIANT *pVar, FILETIME *pTimeStamp, DWORD *pQuality);
...
//
// GetAESourceListFromAreaBrowser(...)
//
// Returns the number of A&E Source names from the connected A&E Server
// If the server does not support the optional AreaBrowser Interfaces,
// this function returns zero.
//
_declspec(dllexport) int  WINAPI GetAESourceListFromAreaBrowser (HANDLE hConnect);

//
// GetAESourceName(...)
//
// Allows user to iterate through the list of source names obtained from
// GetAESourceListFromAreaBrowser().
//

_declspec(dllexport) BOOL  WINAPI GetAESourceName (HANDLE hConnect, int index, char *pBuf, int BufSize);
