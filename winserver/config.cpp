...
#define set_sval(name, value) do { \
    sval = json_string_value(value); \
    slen = json_string_length(value); \
    strncpy(name, sval, slen > sizeof(name) ? sizeof(name):slen); \
    } while(0)

#define set_fval(name, value) do { \
    fval = json_number_value(value); \
    name = fval; \
    } while(0)

#define set_ival(name, value) do {\
...
static inline json_t* get_jstr(const char *ws);
...
int config_file::load_file(const char* name) {
    FILE* fp;
    long size;
...
int config_file::load(const char* s) {
    ...
    sval = json_string_value(value);
    slen = json_string_length(value);
    strncpy(version, sval, slen > sizeof(version) ? sizeof(version):slen);
    ...
    if(json_is_array(conns)) {
        ...
        json_array_foreach(conns, i_conn, conn) {
            ...
            set_ival(cfg_conn.active, value);


            value = json_object_get(conn, "host");
            set_sval(cfg_conn.host, value);

            value = json_object_get(conn, "server");
            set_sval(cfg_conn.server, value);
            ...
            if(json_is_array(groups)) {
                ...
                json_array_foreach(groups, i_group, group) {
                    ...
                    set_ival(cfg_group.active, value);

                    value = json_object_get(group, "group");
                    set_sval(cfg_group.group, value);

                    value = json_object_get(group, "rate");
                    set_ival(cfg_group.rate, value);

                    value = json_object_get(group, "dead_band");
                    set_fval(cfg_group.dead_band, value);
                    ...
                    if(json_is_array(items)) {
                        ...
                        json_array_foreach(items, i_item, item) {
                            ...
                            set_ival(cfg_item.active, value);

                            value = json_object_get(item, "item");
                            set_sval(cfg_item.item, value);

                            value = json_object_get(item, "type");
                            set_sval(cfg_item.type, value);
...
int config_file::merge(const config_file* cfg) {
    ...
    for(i_conn=0; i_conn < cfg->connections.size(); ++i_conn) {
        ...
        for(s_conn=0; s_conn<connections.size(); ++s_conn) {
            ...
            if( strcmp(self_conn.host, cfg_conn.host) ==0 &&
                strcmp(self_conn.server, cfg_conn.server) == 0) {
                // 同一个连接
                find = true;
                break;
            }
        ...
        if(find) {
            ...
            for(i_group=0; i_group<cfg_conn.groups.size(); ++i_group) {
                ...
                for(s_group=0; s_group< self_conn.groups.size(); ++s_group) {
                    ...
                    if(strcmp(self_group.group, cfg_group.group) == 0 ) {
                        find_group = true;
                        break;
                    }
                }
                ...
                if(find_group) {
                    ...
                    for(i_item=0; i_item<cfg_group.items.size(); ++i_item) {
                        ...
                        for(s_item=0; s_item < self_group.items.size(); ++s_item) {
                            ...
                            if(strcmp(self_item.item, cfg_item.item) == 0) {
                                find_item = true;
                                break;
                            }
                        }
...
json_t* get_jstr(const char* ws) {

    json_t* value;
    value = json_string_nocheck(ws);
    return value;
}
