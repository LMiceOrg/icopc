...
// ReadVQT(...)
//
//  DataAccess 3.0 function to read a tag's Value, Quality & Timestamp
//
_declspec(dllexport) HRESULT  WINAPI ReadVQT (HANDLE hConnect, LPCSTR ItemName, DWORD MaxAge,VARIANT *pVar, WORD *pQuality, FILETIME *pTimeStamp);

//
// WriteVQT(...)
//
//  DataAccess 3.0 function to write a tag's Value, Quality & Timestamp
//
_declspec(dllexport) HRESULT  WINAPI WriteVQT (HANDLE hConnect, LPCSTR ItemName, VARIANT Value, WORD Quality, FILETIME TimeStamp);
